...
public class MemberController {
    ...
    public ModelAndView mailSending(HttpServletRequest request, @RequestParam String e_mail, HttpServletResponse response_email) throws IOException  {
        ...
        mv.setViewName("/member/memberWriteForm"); //뷰의이름
        ...
        out_email.println("<script>alert('이메일이 발송되었습니다. 인증번호를 입력해주세요.');</script>");//얘가 안뜨고 있음
        out_email.flush();
        return mv;
	...
	@RequestMapping(value = "checkEmail{dice}", method = RequestMethod.POST)
	public ModelAndView join_injeung(@RequestParam String checkEmail, @PathVariable String dice, HttpServletResponse response_equals) throws IOException {
		
	    System.out.println("입력한 인증번호 : "+checkEmail);
	    System.out.println("전송된 인증번호 : "+dice);
    ...
    //페이지이동과 자료를 동시에 하기위해 ModelAndView를 사용해서 이동할 페이지와 자료를 담음
	    ...
	    mv.addObject("e_mail",checkEmail);//입력한 인증번호
	    
	    if (checkEmail.equals(dice)) {
		        //인증번호가 일치할 경우 인증번호가 맞다는 창을 출력하고 회원가입창으로 이동함
		        mv.setViewName("member/join");
		        
		        mv.addObject("e_mail",checkEmail);
...
//	    ModelAndView mv = new ModelAndView();
//	    mv.addObject("checkEmail",checkEmail);
//	    mv.addObject("dice",dice);

    return mv;//사용자가 입력한 인증번호가 mv에 있음
}
