...
public class CartController {
	...
	@ResponseBody
	public String addGoodsInCart(GoodsDTO goodsDTO, HttpServletRequest request, HttpServletResponse response) {
		...
		cartDTO.setMember_id("admin");
		cartDTO.setGoods_id("S123123");
...
//		cartDTO.setGoods_id(goodsDTO.getGoods_id()); //본 컨트롤러 매개변수인 제품번호(제품 상세정보에서 넘어온 값)
	...
	@RequestMapping(value="/getListFromCartAndGoods", method=RequestMethod.POST)
	public ModelAndView getListFromCartAndGoods(HttpServletRequest request, HttpServletResponse response) {
		...
		session.setAttribute("cartMap", cartMap); //t
